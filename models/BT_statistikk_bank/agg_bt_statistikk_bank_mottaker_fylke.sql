...
mottaker_fylke as (
    select
       ...
       ,mottaker.stat_aarmnd
       ,count(distinct case when mottaker.barn_selv_mottaker_flagg = 0 then mottaker.fk_person1 end) antall --Ekskludere barn selv er mottaker
       ,count(distinct case when mottaker.barn_selv_mottaker_flagg = 1 then mottaker.fk_person1 end) antall_mottaker_barn --Barn selv er mottaker
    from navarende_fylke fylke

    full outer join mottaker_alle mottaker
