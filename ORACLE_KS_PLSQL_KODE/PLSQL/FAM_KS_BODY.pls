 ...
 --v_storedate DATE := sysdate;
    ...
    CURSOR CUR_BARN IS
        ...
        FROM
          ...
          JOIN (
            SELECT
              UR_VEDTAK1.DELYTELSE_ID,
              MAX(UR_VEDTAK1.BEHANDLINGS_ID) KEEP (DENSE_RANK FIRST ORDER BY FAGSAK.TIDSPUNKT_VEDTAK DESC) AS SISTE_VERSJON,
              MAX(FAGSAK.PK_KS_FAGSAK) KEEP (DENSE_RANK FIRST ORDER BY FAGSAK.TIDSPUNKT_VEDTAK DESC)       AS SISTE_FK_KS_FAGSAK
            FROM
              UR_VEDTAK1
              JOIN DVH_FAM_KS.FAM_KS_FAGSAK FAGSAK
              ON UR_VEDTAK1.FAGSAK_ID = FAGSAK.FAGSAK_ID
      ...
      SELECT
        ...
        UR.FAGSAK_ID,
        MAX(UR.SISTE_FK_KS_FAGSAK)                        AS FK_KS_FAGSAK,
        ...
        UR.FK_DIM_TID_MND,
        MAX(VILKAAR.ANTALL_TIMER)                         AS ANTALL_TIMER,
      ...
      FROM
        ...
        ON UR.SISTE_FK_KS_FAGSAK = VILKAAR.FK_KS_FAGSAK
        AND UR.FK_PERSON1_BARN = VILKAAR.FK_PERSON1
 --and ur.stonad_tom >= vilkaar.periode_fom
 --and ur.stonad_tom <= nvl(vilkaar.periode_tom, ur.stonad_tom)
        AND UR.SISTE_DATO_I_PERIODEN >= VILKAAR.PERIODE_FOM
        AND UR.SISTE_DATO_I_PERIODEN <= NVL(VILKAAR.PERIODE_TOM,
        UR.SISTE_DATO_I_PERIODEN)
